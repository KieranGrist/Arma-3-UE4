...
{
	return ClassMember("name", classMemberTabs, member ? member->_VItem._VariableName.ToString() : "");
}
...
FString UItemBase::ClassMember(FString name, int classMemberTabs, TArray<UVirtualItem*> virtualItem)
{
	...
	{
		ret += Quote() + virtualItem[i]->_VItem._VariableName.ToString() + Quote() + Comma(i < virtualItem.Num());;
	}
	ret += ClosedBracketSemiColon() + NewLine();
	return ret;
}
...
FString UItemBase::ClassMember(FString name, int classMemberTabs, TArray<UClothingItem*> clothingItems)
{
	...
	{
		ret += clothingItems[i]->_ClothingItem.MakeString(i < clothingItems.Num());
	}
	ret += ClosedBracketSemiColon() + NewLine();
	return ret;
}
